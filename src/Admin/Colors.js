...
export default function Colors(props) {
    ...
    const handleSubmit = async () => {
        ...
        var result = await postDataAndImage('colors/colorsubmit', formData, { headers: { "content-type": "multipart/formData" } })
    
        if (result) {
          Swal.fire({
            text: 'Colors Added Successfully',
            imageUrl: '/logo.jpg',
        ...
        <div className={classes.root}>
            <div className={classes.subdiv}>
                <Grid container spacing={2}>
                    ...
                    <Grid item xs={12}>
                        <div>
                            <Button startIcon={<List />} onClick={()=>handleClick()} variant="contained">Colors List</Button>
                        </div>
                    </Grid>
                    <Grid item xs={6}>
                    ...
                    <Grid item xs={12}>
                        <TextField fullWidth onChange={(event) => setcolor(event.target.value)} variant="outlined" label="Color" />
                    </Grid>
